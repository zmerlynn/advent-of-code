...
fn run(input: String) -> String {
    ...
    let race_times = parse_ints(&input_iter.next().unwrap().split(":").collect_tuple::<(&str,&str)>().unwrap().1.replace(" ", ""));
    let race_record_distances = parse_ints(&input_iter.next().unwrap().split(":").collect_tuple::<(&str,&str)>().unwrap().1.replace(" ", ""));
